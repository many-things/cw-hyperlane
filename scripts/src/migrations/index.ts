...
export async function runMigrations(network: string, dryRun: boolean) {
  ...
  // find initials
  const availableInitials = Object.keys(migraiotnMap)
    .map((key) => migraiotnMap[key])
    .filter((obj) => obj.after === "");
    ...
    Object.keys(migraiotnMap).forEach((key) => {
      ...
      if (obj.after === current.name) {
        next === undefined
          ? (next = obj)
          : (() => {
              throw new Error("There must be one next migration");
            })();
      }
    });
  ...
  const migStartAt =
    migrationOrder.findIndex((obj) => obj.name === ctx.latestMigration) + 1;
