...
import { Contract, ContractContext, HplHubInstantiateMsg } from "../types";
import { getWasmPath } from "../load_wasm";
import fs from "fs";

export default class HplHub implements Contract {
  contractName: string = "hpl_hub"
  ...
  instantiate = async(msg: any): Promise<ContractContext> => {
    const instantiateMsg = msg as HplHubInstantiateMsg;
