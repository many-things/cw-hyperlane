use cosmwasm_std::{ensure, ensure_eq, DepsMut, Event, MessageInfo, Response};
...
mod test {
    ...
    fn test_pause() {
        ...
        let unauthorized = pause(deps.as_mut(), mock_info(ADDR2_VALUE, &[])).unwrap_err();
        assert!(matches!(unauthorized, ContractError::Unauthorized {}));

        // already paused
        PAUSE.save(deps.as_mut().storage, &true).unwrap();
        let already_paused = pause(deps.as_mut(), mock_info(ADDR1_VALUE, &[])).unwrap_err();
        assert!(matches!(already_paused, ContractError::Paused {}));

        // success
        PAUSE.save(deps.as_mut().storage, &false).unwrap();
        let res = pause(deps.as_mut(), mock_info(ADDR1_VALUE, &[])).unwrap();
    ...
    fn test_unpause() {
        ...
        let unauthorized = unpause(deps.as_mut(), mock_info(ADDR2_VALUE, &[])).unwrap_err();
        assert!(matches!(unauthorized, ContractError::Unauthorized {}));

        // not paused
        PAUSE.save(deps.as_mut().storage, &false).unwrap();
        let not_paused = unpause(deps.as_mut(), mock_info(ADDR1_VALUE, &[])).unwrap_err();
        assert!(matches!(not_paused, ContractError::NotPaused {}));

        // success
        PAUSE.save(deps.as_mut().storage, &true).unwrap();
        let res = unpause(deps.as_mut(), mock_info(ADDR1_VALUE, &[])).unwrap();
